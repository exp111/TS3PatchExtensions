...
const char *ts3plugin_description() {
	return "A example plugin which uses the hook api & Qt";
}
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon)
{
	...
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_GLOBAL, MENU_ID_GLOBAL_1, "Download", "");
	END_CREATE_MENUS;
...
void download()
{
	...
	downloader->doDownload("https://raw.githubusercontent.com/exp111/TS3PatchExtensions/master/customBadges/badges.csv", "D:/badges.csv");
}
...
int ts3plugin_processCommand(uint64 serverConnectionHandlerID, const char* command) 
{
	...
	{
		functions.printMessageToCurrentTab(("IsDeleted: " + to_string(downloader == nullptr)).c_str());
		return 0;
	}
	download();
	return 0; //0 = We handled the command
}
